...
class StoryRepository @Inject constructor(
    ...
    suspend fun storiesWithLocation(): Flow<ApiResponse<StoryResponse>> {
        return flow {
            try {
                ...
                val response = ApiConfig.storyService(client).stories(location = 1)
    ...
    suspend fun storyDetail(id: String): Flow<ApiResponse<StoryDetailResponse>> {
        return flow {
            try {
                emit(ApiResponse.Loading)
                val response = ApiConfig.storyService(client).storyDetail(id)
                if (response.code() == 200) {
                    emit(ApiResponse.Success(response.body()))
                } else {
                    val errorBody = JSONObject(response.errorBody()!!.string())
    ...
    fun storiesForList(): Flow<PagingData<Story>> {
        ...
        return Pager(
            ...
            remoteMediator = StoryRemoteMediator(storyDatabase, ApiConfig.storyService(client)),
            pagingSourceFactory = {
    ...
    suspend fun addStory(
        ...
        return flow {
            try {
                ...
                val response = ApiConfig.storyService(client)
